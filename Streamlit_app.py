...
if st.button("Predict"):
    ...
    # Display results
    if pred == 1:
        st.error(f"Prediction: Churn=1" if prob_churn is not None else "Prediction: Churn")
        st.write(f"Probability:  {prob_churn:.4f}")
    else:
        st.success(f"Prediction: Stay=0 " if prob_churn is not None else "Prediction: Stay")
